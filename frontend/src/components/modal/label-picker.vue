...
export default {
  ...
  methods: {
    set(label) {
      const labels = this.labelsActive
        ? JSON.parse(JSON.stringify(this.labelsActive))
        : [];
      const idx = this.labelsActive.findIndex(
  ...
  computed: {
    ...
    isActiveMap() {
      const res = this.labels.map(
        label =>
          this.labelsActive.findIndex(
