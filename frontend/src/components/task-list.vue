  ...
  <section class="task-list" :ref="taskListData.id">
    ...
    <div class="title">
      <input
        placeholder="Title"
        v-if="editTitleMode"
        class="edit-title-list"
        type="text"
        v-model="copyTitle"
        ref="editTitle"
        @keydown="onKeyEditTitle"
        @blur="blurEditTitle"
      />
      <h2 v-else @click="startEditTitle">{{taskListData.title}}</h2>
      <button @click="barIsOpen = true">
    ...
    <div class="list-items">
      <draggable
        draggable=".task-preview-container"
        v-model="tasks"
        v-bind="dragOptions"
        @end="endMove"
      >
...
export default {
  ...
  methods: {
    startAddTaskMode() {
      ...
      setTimeout(() => {
        this.$refs.inputTxtAddTask.focus();
      }, 0);
    },
    closeAddTaskMode() {
      setTimeout(() => {
